...
BEGIN
	...
	begin try
		...
		FROM ( 
			SELECT rad.*, 
				   RANK() OVER (PARTITION BY rad.code ORDER BY rad.agreement_date DESC) AS row_num 
			FROM ( 
				select  @p_user_id as user_name
						,dmt.code
						,case
				...
				where   dmt.movement_status = 'ON TRANSIT'
						...
						and cast(dmt.movement_date as date) <= @p_as_of_date
						and dm.branch_code = case when  @p_branch_code = 'ALL' then dm.branch_code else  @p_branch_code end
			) rad 
		) rad 
		WHERE rad.row_num = 1

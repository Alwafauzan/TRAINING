USE [IFINDOC]
GO
/****** Object:  StoredProcedure [dbo].[xsp_rpt_adh_collateral]    Script Date: 10/02/2025 14:41:25 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
--Created, Jeff at 11-09-2023
alter PROCEDURE [dbo].[xsp_rpt_adh_collateral_backup]
(
...
BEGIN
	...
	begin try
		...
		select	@p_user_id
				...
				,ama.agreement_date
				,nama.name
				,rank() over (partition by dmt.code order by ama.agreement_date desc) as row_num
		from	dbo.document_main dm
				left join dbo.document_movement_detail dmd on dmd.document_code = dm.code
				left join dbo.document_movement dmt on dmt.code = dmd.movement_code
				left join ifinams.dbo.asset ast on ast.code = dm.asset_no
		...
		where	dmt.movement_status = 'ON TRANSIT' --not in ('HOLD','CANCEL')
				...
				and not exists (
					...
					from	(
								select		doc_code
											, trx_date
											, rank() over (partition by doc_code order by trx_date desc) as row_num
								from		dbo.rpt_adh_collateral_detail
							) rd
					where	rd.doc_code = dmt.code
					and		rd.trx_date > dmt.movement_date
					and		rd.row_num = 1
				)
...
FROM (
    ...
    FROM dbo.table_name initial
    WHERE initial.USER_ID = 'fauzan'
) rad
WHERE rad.row_num = 1